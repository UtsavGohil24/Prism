...
export default function HistoryPage() {
  ...
  // No "all repos" option anymore — a single repo is always selected once
  // reports load, so PR numbers on the chart are always unique.
  ...
  // Auto-select a repo once reports load (or if the previously selected
  // repo no longer has any reports). There's no "all" fallback anymore,
  // so something must always be picked when repos are available.
  useEffect(() => {
  ...
  // Filter + deduplicate (by diff_hash) reports for the selected repo
  const filteredReports = useMemo(() => {
    if (!selectedRepo) return []
    const subset = reports.filter(r => r.repo === selectedRepo)
  ...
  // Chart data: all PRs (within the selected repo), or only the selected PR's versions.
        ...
        <div className="pt-20 px-4 sm:px-6 lg:px-8 py-8 flex-1 flex flex-col justify-between">
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              ...
              {/* Repo Filter — single repo only, no "All Repos" option */}
              {repos.length > 0 && (
                          ...
                          <LineChart data={chartData} margin={{ top: 10, right: 30, left: -10, bottom: 0 }}>
                            ...
                            <XAxis
                              ...
                              tickFormatter={(i) => {
                                ...
                                // Overview chart (single repo selected): PR numbers are
                                // unique within a repo, so plain "#N" is unambiguous.
                                return `#${d.pr}`
                              }}
